...
function filter() {
  ...
  const eventDate = new Date(
    document
      .querySelector('.eventRegistrationInfoEndDate .infoText')
      .textContent.trim()
      .split('-')[0]
  );
  ...
  const hasLTF = Array.from(
    document.querySelectorAll('strong.labelTitle.paymentTitle label')
  )
    .map((element) => element.textContent.trim())
    .some((label) => label.includes('LTF'));

  document
    .querySelectorAll('.eventRegistrationTypeRadioWrapper')
    .forEach((item) => {
      ...
      if (isEarlyBird == undefined) {
        ...
        if (
          (label.includes('Early-Bird') && !isEarlyBird) ||
          (!label.includes('Early-Bird') && isEarlyBird)
        ) {
          item.style.display = 'none';
        }

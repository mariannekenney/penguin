...
function removeUnselected() {
  document.querySelectorAll('.sectionOuterContainer').forEach((section) => {
    let hasContent =
      Array.from(section.querySelectorAll('.fieldBody span')).filter(
        (span) => span && span.textContent
      ).length > 0;

    hasContent =
      hasContent || section.querySelectorAll('.fieldBody .checked').length > 0;
  ...
  const isEquipmentOnly = Array.from(
    document.querySelectorAll('.infoColumn')
